...
@db
def create_user(email, password, last_login_attempt, uuid, db):
    db_user = models.User(email=email, password=password, last_login_attempt=last_login_attempt, uuid=uuid)
    db.add(db_user)
    db.flush()
...
def delete_user(user, db):
    user_tokens = db.query(models.Token).filter(models.Token.user_id==user.uuid)
...
def get_user_tokens(user, db):
    return db.query(models.Token).filter(models.Token.user_id == user.uuid).all()
